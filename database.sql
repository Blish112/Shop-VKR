...
drop table if exists product_prop_value;

create table product_prop_value (
    ...
    foreign key (product_prop_id) references product_props (id),
    foreign key (product_id) references products (id)
);

alter table product_props drop column product_id;
alter table product_props drop column value;

alter table product_props add column category_id integer;

ALTER TABLE ONLY public.product_props
    ADD CONSTRAINT category_prop_id_fkey FOREIGN KEY (category_id) REFERENCES public.categories(id) ON UPDATE CASCADE ON DELETE CASCADE;